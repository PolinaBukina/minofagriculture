...
//     // Загрузка данных лекции (можно заменить на реальный API-запрос)
//     useEffect(() => {
//         // Mock данные
...
//                 <Breadcrumbs
//                     items={[
//                         { label: 'Главная', path: '/listener' },
//                         { label: 'Архив лекций', path: '/archive' },
//                         { label: `${lecture.title}`, path: `/archive/lecture/${id}` }
//                     ]}
//                 />
...
//                                 Посмотреть полный текст лекции
//                             </button>
//                             {showFullText && (
//                                 <div className={commonStyles.LectureFullText}>
//                                     {lecture.content?.original || 'Текст лекции недоступен'}
//                                 </div>
//                             )}
//                         </div>
//                     </div>
...
//                     <div className={commonStyles.listItemLecture}>
//                         <h2>{language === 'en' ? 'Translated text (ENG)' : language === 'fr' ? 'Texte traduit (français)' : '翻译文本（中文'}</h2>
//                         <div className={commonStyles.ItemLecture}>
//                             {showFullText && (
//                                 <div className={commonStyles.LectureFullText}>
...
//                             // onClick={() => setShowFullText(!showFullText)}
//                             >
//                                 {/* {showFullText ? 'Скрыть перевод' : 'Показать перевод'} */}
//                                 {language === 'en' ? 'View the full text of the lecture' : language === 'fr' ? 'Voir le texte intégral de la conférence' : '查看讲座全文'}
//                             </button>
//                         </div>
//                     </div>
//                 </div>
    ...
    const getBreadcrumbs = () => {
        if (fromArchive) {
            return [
                { label: 'Главная', path: '/' },
                { label: 'Архив лекций', path: '/archive' },
                { label: lecture?.title || 'Лекция', path: `/archive/lecture/${id}` }
            ];
        } else {
            return [
                { label: 'Главная', path: '/' },
                { label: 'Активные лекции', path: '/active-lectures' },
                { label: lecture?.title || 'Лекция', path: `/lecture/${id}` }
            ];
        }
    };
                ...
                <div className={commonStyles.infoCardLecture}>
                    ...
                    <div className={commonStyles.listItemLecture}>
                        ...
                        <div className={commonStyles.ItemLecture}>
                            ...
                            >
                                {showFullText ?
                                    (language === 'en' ? 'Hide translation' :
                                        language === 'fr' ? 'Masquer la traduction' :
                                            '隐藏翻译') :
                                    (language === 'en' ? 'Show translation' :
                                        language === 'fr' ? 'Afficher la traduction' :
                                            '显示翻译')}
                            </button>
                        </div>
                    </div>
